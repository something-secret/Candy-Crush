...
function horizontal(x, y) {
  ...
  for (var i = 0; i < 10; i++) {
    ...
    if (board[x][i] == "colour_bomb")
      colourBomb(returnColour(board[x][i]), x, i, normal);
    bufferBoard[x][i] = "0";
    score = score + 10;
  }
}
...
function randomGen() {
  ...
  if (level >= 3) {
    ...
    board[x][y] = returnColour(board[x][y]) + "_horizontal";
  }
  if (level >= 3) {
    var x = Math.floor(Math.random() * 10);
    var y = Math.floor(Math.random() * 10);
    board[x][y] = returnColour(board[x][y]) + "_vertical";
  }
  if (level >= 4) {
    var x1 = Math.floor(Math.random() * 10);
    var y1 = Math.floor(Math.random() * 10);
    board[x1][y1] = returnColour(board[x1][y1]) + "_packet";
    var x2 = Math.floor(Math.random() * 10);
    var y2 = Math.floor(Math.random() * 10);
    board[x2][y2] = returnColour(board[x2][y2]) + "_packet";
  }
  if (level >= 6) {
    var x = Math.floor(Math.random() * 10);
    var y = Math.floor(Math.random() * 10);
...
function render() {
  for (var i = 0; i < 10; i++) {
    for (var j = 0; j < 10; j++) {
      if (board[i][j] == "0") {
        document.getElementById(i + "-" + j).innerHTML = '<image src="assets/break.png" class="cell-contents">';
      } else {
...
function levelGen() {
  if (totalMoves == 0 || score >= requiredScore) {
    if (iDoNothing == 1)
      alert("Yay, you completed the level, now go to the next one");
    count = 0;
    score = 0;
    iDoNothing = 1;
    level++;
    totalMoves = Math.round((((Math.random() + 2) * (Math.random() + 1) * level) + level) * 4);
    requiredScore = (totalMoves * 30) + Math.round((Math.random()) * 200);
    ...
    render();
  } else if (totalMoves - count == 0 && totalMoves != 0) {
    alert("Ah, You failed, now imma reset your board, hehe");
    level = 0;
    score = 0;
    count = 0;
    totalMoves = 0;
